...
public class BrancheDetail {
    ...
    public Response newBook(Branche new_bra) throws Exception {
        ...
        try {
            ...
            response.entity(BrancheDAL.getById(new_bra.getId()));
        } catch (Exception e) {
    ...
    public Response updateBook(Branche upd_bra) throws IOException {
        ...
        try {            
        	...
        	if(upd_bra.getId() == "" || upd_bra.getId() == null)
        		throw new Exception("Update ohne ID nicht möglich");
        	
        	BrancheDAL.update(upd_bra.getId(), upd_bra);
        	response.status(Response.Status.OK);
            response.entity(BrancheDAL.getById(upd_bra.getId()));
        } catch (Exception e) {
            response.status(Response.Status.BAD_REQUEST);
            response.entity("[ERROR] " + e.getMessage());
        }
